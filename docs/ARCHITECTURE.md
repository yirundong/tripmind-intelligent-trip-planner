...
本文档为 TripMind 智能旅行规划系统的项目架构说明，涵盖模块划分、架构图、需求分析、核心实现与测试方案，供后续开发与维护参考。
...
6. 观察进度条和 Agent 阶段：需求解析 → 景点搜索 → 天气查询 → 酒店查询 → 行程生成 → 结构校验 → 数据保存。
7. 生成成功后进入结果页，查看行程概览、预算、地图、每日景点、酒店、餐饮和天气。
8. 进入编辑模式，调整一个景点顺序或修改描述并保存。
9. 验证导出 PNG 或 PDF。
10. 进入"我的行程"，确认刚才保存的行程记录。
11. 登录管理员账号，验证任务日志、成功率、热门目的地和用户管理。
