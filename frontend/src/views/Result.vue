    ...
    <div v-if="tripPlan" class="content-wrapper">
      ...
      <div class="main-content">
        <section class="result-summary">
          <div>
            <div class="page-kicker">TRIP PLAN</div>
            <h1>{{ tripPlan.city }}旅行方案</h1>
            <p>{{ tripPlan.start_date }} 至 {{ tripPlan.end_date }}，共 {{ tripPlan.days.length }} 天</p>
          </div>
          <div class="summary-metrics">
            ...
            <div class="summary-metric">
              <span>预算总额</span>
              <strong>{{ tripPlan.budget ? `¥${tripPlan.budget.total}` : '-' }}</strong>
            </div>
            <div class="summary-metric">
              <span>天气天数</span>
              <strong>{{ tripPlan.weather_info?.length || 0 }}</strong>
            </div>
          </div>
        </section>
...
onMounted(async () => {
  ...
  tripId.value = storedTripId || null
  if (data) {
    tripPlan.value = JSON.parse(data)
  } else if (tripId.value) {
    const detail = await fetchTrip(tripId.value)
    tripPlan.value = detail.plan_data
  ...
  try {
    ...
    cards.forEach((card) => {
      ...
      try {
        ...
        cardEl.style.setProperty('border-radius', '6px')
...
const initMap = async () => {
  try {
    ...
    if (!jsApiKey) {
      throw new Error('未配置 VITE_AMAP_WEB_JS_KEY')
    }
    ...
    if (securityJsCode) {
      ...
      console.warn('未配置 VITE_AMAP_SECURITY_JS_CODE, 新申请的高德 JSAPI Key 可能无法通过鉴权')
    }

    const AMap = await AMapLoader.load({
      key: jsApiKey,  // 高德地图Web端(JS API) Key
      version: '2.0',
    ...
    message.error(`地图加载失败: ${error?.message || '请检查高德JS API Key和安全密钥'}`)
  }
}
...
.side-nav :deep(.ant-menu-item-selected span) {
  color: #0f766e;
}
